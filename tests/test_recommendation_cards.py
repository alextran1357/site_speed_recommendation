    ...
    def test_help_request_uses_recommendation_evidence_and_correct_scope(self):
        ...
        for scope in ("URL", "Origin", None):
            with self.subTest(scope=scope):
                ...
                self.assertIn("actual transfer size", message)
                self.assertIn("srcset/sizes", message)
                self.assertIn("estimated byte savings are not guaranteed time savings", message)
